...
textTupleWin = ['Умничка!','Слушай, а у тебя неплохо получается','Как на счёт ещё одного раза, я тебя обыграю!)','Красава ;)','Я могу лучше!!! Давай ещё разок) ']
textTupleLose = ['Тебе меня не обыграть!!!','ХАХАХА, попробуй снова)','Ты проиграл, смирись...','Может получится в следующий раз ;)','Не расстраивайся, давай ещё разик, а?)']
class GameItem:
...
def compareHand(playerHand, PCHand):
    ...
    if playerHand.name == PCHand.name:
        ...
        if winCounter > len(playerHand.power) / 2:
            text_result = rnd.choice(textTupleWin)
        else:
            text_result = rnd.choice(textTupleLose)
    print(text_result)
    print()
    return text_result
'''
